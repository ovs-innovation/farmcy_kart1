...
```
             ...
             ┌───────────────────────────────┼───────────────────────────────┐
             ▼                               ▼                               ▼
   https://farmacykart.com         https://api.farmacykart.com     https://admin.farmacykart.com
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **`backend`** | Node.js / Express | `5000` | `5000` | `http://127.0.0.1:5000/` | `https://api.farmacykart.com` |
| **`frontend`** | Next.js 14 Standalone | `3000` | `3000` | `http://127.0.0.1:3000/` | `https://farmacykart.com` |
| **`admin`** | Vite React SPA + Nginx | `80` | `4100` | `http://127.0.0.1:80/healthz` | `https://admin.farmacykart.com` |
...
3. **Frontend & Admin Build-Time Inlining**:
   ...
   - **Fix**: Multi-stage Dockerfiles allow `.env` into the build context during `npm run build` without exposing `.env` in the runner images. Domain build args default to production `farmacykart.com` domains.
4. **NextAuth Production Domain**:
   - Initial Compose file set `NEXTAUTH_URL=http://localhost:3000`.
   - **Fix**: Updated to `https://farmacykart.com` and removed empty `NEXTAUTH_SECRET` override so `frontend/.env` runtime secret is preserved.
...
FRONTEND_URL=https://farmacykart.com
STORE_URL=https://farmacykart.com
ADMIN_URL=https://admin.farmacykart.com

# Email Provider (Resend or SMTP)
RESEND_API_KEY=re_your_resend_api_key_here
# Alternative SMTP:
# EMAIL_USER=notifications@farmacykart.com
...
NEXT_PUBLIC_API_BASE_URL=https://api.farmacykart.com/api
NEXT_PUBLIC_STORE_DOMAIN=https://farmacykart.com

# NextAuth Configuration
NEXTAUTH_URL=https://farmacykart.com
...
```bash
# API & Domain Configuration
VITE_APP_API_BASE_URL=https://api.farmacykart.com/api
VITE_APP_STORE_DOMAIN=https://farmacykart.com
VITE_APP_ADMIN_DOMAIN=https://admin.farmacykart.com
...
### 1. Storefront (`farmacykart.com` & `www.farmacykart.com`)
```nginx
server {
    server_name farmacykart.com www.farmacykart.com;
    ...
    ssl_certificate /etc/letsencrypt/live/farmacykart.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/farmacykart.com/privkey.pem;
}
```

### 2. Backend API (`api.farmacykart.com`)
```nginx
server {
    server_name api.farmacykart.com;
    ...
    listen 443 ssl http2;
    ssl_certificate /etc/letsencrypt/live/api.farmacykart.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.farmacykart.com/privkey.pem;
}
```

### 3. Admin Panel (`admin.farmacykart.com`)
```nginx
server {
    server_name admin.farmacykart.com;
    ...
    listen 443 ssl http2;
    ssl_certificate /etc/letsencrypt/live/admin.farmacykart.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/admin.farmacykart.com/privkey.pem;
}
```
```
