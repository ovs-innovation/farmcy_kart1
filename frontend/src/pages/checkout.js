...
const Checkout = () => {
  ...
  const { data: shippingAddressesResponse, refetch: refetchAddresses } =
    useQuery({
      queryKey: ["shippingAddresses", userInfo?._id],
      queryFn: async () => {
        if (!userInfo?._id) return null;
        const response = await CustomerServices.getShippingAddress({
          userId: userInfo._id,
        });
  ...
  React.useEffect(() => {
    const fetchLocation = async () => {
      if (
        ...
        /^\d+$/.test(addressForm.zipCode)
      ) {
        try {
          const response = await fetch(
            `https://api.postalpincode.in/pincode/${addressForm.zipCode}`,
          );
          ...
          if (
            ...
            notifySuccess(
              `Location fetched: ${postOffice.District || postOffice.Block || postOffice.Name}, ${postOffice.State}`,
            );
          }
        } catch (error) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          const geocodeData = await LocationServices.getReverseGeocode({
            lat: latitude,
            lng: longitude,
          });
    ...
    try {
      ...
      if (response.success || response.message) {
        ...
        if (addressForm.isDefault || shippingAddresses.length === 0) {
          const updatedResponse = await CustomerServices.getShippingAddress({
            userId: userInfo._id,
          });
                        ...
                        <div className="space-y-3 max-h-[300px] overflow-y-auto scrollbar-hide">
                          {shippingAddresses.map((address) => {
                            ...
                            const locationDisplay =
                              locationText.length > 25
                                ? locationText.substring(0, 25) + ".."
                                : locationText;
                      ...
                      {couponInfo.couponCode ? (
                        ...
                        <div className="space-y-3">
                          <p className="text-xs text-gray-500 italic mb-2">
                            * Applying a new coupon will replace the existing
                            coupon.
                          </p>
                          {availableCoupons && availableCoupons.length > 0 ? (
                            <>
                              ...
                              >
                                <option value="">{t("Select a coupon")}</option>
                                {availableCoupons.map((coupon) => (
                                  ...
                                  >
                                    ...
                                    {Number(coupon.minimumAmount || 0).toFixed(
                                      2,
                                    )}
                                  </option>
                                ))}
                              ...
                              <div className="flex flex-col sm:flex-row items-start justify-end">
                                {isCouponAvailable ? (
                                  ...
                                  <button
                                    disabled={
                                      isCouponAvailable || !selectedCouponCode
                                    }
                ...
                <div className="mt-6 bg-gray-50 rounded-lg p-4">
                        ...
                        <div className="flex-1">
                          <div className="relative">
                            ...
                            <div className="absolute inset-y-0 right-0 flex items-center pointer-events-none">
                              <svg
                                className="w-4 h-4 text-store-600"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                  ...
                  <div className="mt-4 flex items-start gap-2">
                    ...
                    <label
                      ...
                      className="text-xs sm:text-sm text-gray-900 font-semibold cursor-pointer leading-relaxed"
                    >
                      By placing the order, you agree to our{" "}
                      <Link
                        href="/terms-and-conditions"
                        className="text-store-700 hover:text-store-800 hover:underline font-bold"
                      >
                        Terms & Conditions
                      </Link>{" "}
                      and{" "}
