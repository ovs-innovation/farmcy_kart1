...
const updateOrder = async (req, res) => {
  try {
    ...
    // --- Referral Logic on First Order Delivery ---
    if (status === "Delivered" && previousStatus !== "Delivered") {
      try {
        ...
        const pendingReferral = await Referral.findOne({
          referredUser: updatedOrder.user,
          status: "pending",
        });

        if (pendingReferral) {
          // Verify this is actually their FIRST delivered order
          const deliveredOrdersCount = await Order.countDocuments({
            user: updatedOrder.user,
            status: "Delivered",
          });

          if (deliveredOrdersCount === 1) { // Current order is the first one
            
            // 1. Generate ₹50 OFF coupon for the new user
            const discountCouponCode = `REF50-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
            const newDiscountCoupon = new Coupon({
              title: { en: "Referral Sign-Up Bonus" },
              couponCode: discountCouponCode,
              endTime: dayjs().add(1, "year").toDate(),
              minimumAmount: 100,
              discountType: { type: "fixed", value: pendingReferral.discountAmount || 50 },
              status: "hide", // Hidden from public coupon screen
            });
            await newDiscountCoupon.save();

            // 2. Generate ₹100 OFF coupon for the referrer
            const rewardCouponCode = `REF100-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
            const newRewardCoupon = new Coupon({
              title: { en: "Referral Reward" },
              couponCode: rewardCouponCode,
              endTime: dayjs().add(1, "year").toDate(),
              minimumAmount: 200,
              discountType: { type: "fixed", value: pendingReferral.rewardAmount || 100 },
              status: "hide", // Hidden from public coupon screen
            });
            ...
            pendingReferral.discountCoupon = newDiscountCoupon._id;
            pendingReferral.rewardCoupon = newRewardCoupon._id;
            ...
            console.log(`[Referral] Processed rewards for referral ${pendingReferral._id}`);
          }
        }
      } catch (refErr) {
        console.error("[Referral] Error processing referral rewards:", refErr);
      }
    }
    // ---------------------------------------------
